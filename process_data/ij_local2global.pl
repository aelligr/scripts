...
if ($#ARGV!=7) {
  print "Usage: ij_local2global.pl ie_tot je_tot nboundlines nprocx nprocy my_cart_id i_local j_local\n";
...
@loc_isubpos = @{ $isubpos[$my_cart_id] };
$i_global = $loc_isubpos[0]+($i-$nboundlines-1);
$j_global = $loc_isubpos[1]+($j-$nboundlines-1);
printf("   PE      i      j  i_global  j_global\n");
printf("%5d %6d %6d %9d %9d\n",$my_cart_id,$i,$j,$i_global,$j_global);
